...
void SphereModel::initialize_sphere(std::vector<Vector3D> &sphere_points, const unsigned int depth) {
    ...
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(0, 2, 1)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(0, 3, 2)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(0, 4, 3)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(0, 5, 4)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(0, 1, 5)));
    
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(1, 2, 7)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(2, 3, 8)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(3, 4, 9)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(4, 5, 10)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(5, 1, 6)));
    
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(1, 7, 6)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(2, 8, 7)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(3, 9, 8)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(4, 10, 9)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(5, 6, 10)));
    
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(6, 7, 11)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(7, 8, 11)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(8, 9, 11)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(9, 10, 11)));
    m_mesh->m_indices.push_back(_SharedPtr<TriangleIndice>(new TriangleIndice(10, 6, 11)));
    
    
    for(int i = 0; i < 12; i++){
        
        m_mesh->m_points.push_back(_SharedPtr<Vector3D>(new Vector3D(icosaVertices[i])));
...
void SphereModel::subdivide( Vector3D v1,  Vector3D v2, Vector3D v3, std::vector<_SharedPtr<Vector3D > > &sphere_points, const unsigned int depth) {
    
    if(depth == 0) {
        int index = (int)m_mesh->m_points.size();
        m_mesh->m_points.push_back(_SharedPtr<Vector3D>(new Vector3D(v1)));
        m_mesh->m_points.push_back(_SharedPtr<Vector3D>(new Vector3D(v2)));
        m_mesh->m_points.push_back(_SharedPtr<Vector3D>(new Vector3D(v3)));

        m_mesh->m_colors.push_back(_SharedPtr<ColorIndice>(new ColorIndice(index, '~', COLOR_BLUE, COLOR_BLACK)));
        m_mesh->m_colors.push_back(_SharedPtr<ColorIndice>(new ColorIndice(index+1, '^', COLOR_WHITE, COLOR_BLACK, 0)));
	if(index%2)
