...
public:
    ...
    static Opentxs* Instance();

    bool start(std::vector<std::string> arguments);
    bool stop();

    int run(std::string command);
    void handleCommand(std::string command, std::vector<std::string> arguments);
...
private:
    ...
    Opentxs(Opentxs const&){};
    Opentxs& operator=(Opentxs const&){};

    static Opentxs* m_pInstance;
    ...
    opentxs::OTVariable* setGlobalVar(opentxs::OT_ME& madeEasy, const std::string& name,
                             const std::string& value);
    int processCommand(opentxs::OT_ME& madeEasy, AnyOption *opt);
    ...
    bool noPrompt, echoExpand, echoCommand;

    _SharedPtr<AnyOption> m_opt;
};
